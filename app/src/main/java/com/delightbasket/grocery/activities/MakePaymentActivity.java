...
public class MakePaymentActivity extends AppCompatActivity implements PaymentResultListener {
    ...
    private void createOrderOnRazorPay() {
        ...
        call.enqueue(new Callback<RazorpayOrderIdResponse>() {
            ...
            public void onFailure(Call<RazorpayOrderIdResponse> call, Throwable t) {
                Toast.makeText(MakePaymentActivity.this, t.getMessage(), Toast.LENGTH_LONG).show();
            }
    ...
    private void startPayment(String orderId) {
        ...
        try {
            ...
            options.put("image", ContextCompat.getDrawable(MakePaymentActivity.this, R.drawable.logo_app_png));
    ...
    public void onPaymentError(int i, String s) {
        ...
        setResult(2231, intent);
        finish();
    }

    private void updateRazorPayPayment(String razorPayPaymentID) {
        Call<RazorpaySuccessResponse> call = service.getRazorPaymentSuccess(Const.DEV_KEY, token, razorPayPaymentID, orderId, amount);
        call.enqueue(new Callback<RazorpaySuccessResponse>() {
            ...
            public void onResponse(Call<RazorpaySuccessResponse> call, Response<RazorpaySuccessResponse> response) {
                if (response.code() == 200 && response.body().getStatus() == 200) {
                    Toast.makeText(MakePaymentActivity.this, response.body().getMessage(), Toast.LENGTH_LONG).show();
                    Intent intent = new Intent();
